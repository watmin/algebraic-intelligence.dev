...
I should be explicit about something: every file in every repository — the Python library, the Rust port, the XDP/eBPF scrubber, the traffic generator, this site — is entirely LLM-generated. I wrote zero code. All of it came from prompting: describing what I wanted, reviewing what came back, correcting course, pushing further. The domain knowledge and the architectural decisions are mine. The code is not. That's a statement about what these tools make possible when you bring deep intuition and a clear mental model to the table.
